 ...
 */
export function convertWebmToMp4(options) {
    ensureFfmpegConfigured();
    return new Promise((resolve, reject) => {
        if (!isFfmpegAvailable()) {
            reject(new FfmpegConversionError('FFmpegの実行ファイルが見つかりません。アプリの再インストールをお試しください。'));
            return;
        }
...
function toFriendlyFfmpegError(error) {
