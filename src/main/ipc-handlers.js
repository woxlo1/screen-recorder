...
function toAppError(code, error) {
...
export function registerIpcHandlers() {
    ...
    ipcMain.handle(IpcChannels.StartRecording, async (_event, payload) => {
    ...
    // --- 録画停止 → WebMファイルへの書き込み -----------------------------
    ipcMain.handle(IpcChannels.StopRecording, async (_event, payload) => {
        if (!recordingStateManager.isActive()) {
    ...
    ipcMain.handle(IpcChannels.SaveVideo, async (event, request) => {
        ...
        try {
            ...
            if (request.save.format === 'mp4') {
                if (!isFfmpegAvailable()) {
                    const appError = toAppError('FFMPEG_NOT_AVAILABLE', new Error('FFmpegの実行ファイルが見つかりません'));
                ...
                try {
                    ...
                    sendProgress({ requestId: request.requestId, phase: 'failed' });
                    const message = error instanceof FfmpegConversionError
                        ? error.message
    ...
    // --- 保存先フォルダ選択 -------------------------------------------------
    ipcMain.handle(IpcChannels.SelectFolder, async (event) => {
    ...
    ipcMain.handle(IpcChannels.LoadSettings, async () => {
    ...
    ipcMain.handle(IpcChannels.DeleteRecordingHistoryItem, async (_event, payload) => {
        persistentStore.deleteHistoryItem(payload.id);
        return { success: true };
    });
    ipcMain.handle(IpcChannels.OpenFileLocation, async (_event, payload) => {
        try {
