...
import { combineStreams, getDisplayCaptureStream, getMicrophoneStream, stopStream, } from './mediaStream';
...
export function useRecorderController() {
    ...
    const rawVideoStreamRef = useRef(null);
     ...
     */
    const handleUnexpectedStop = useCallback((reason) => {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            const wantsSystemAudio = audioSettings.systemAudioEnabled &&
            ...
            throw new Error(toFriendlyMediaError(error, '画面録画の権限が許可されていない可能性があります。'));
        }
        ...
        try {
            ...
            throw new Error(toFriendlyMediaError(error, 'マイクの権限が許可されていない可能性があります。'));
        }
...
export function getResolutionDimensions(preset) {
 ...
 */
function toFriendlyMediaError(error, permissionHint) {
